...
<template>
  ...
  >
    <NuxtLink :to="path" class="grid grid-cols-1 sm:grid-cols-10 gap-1">
      ...
      <div class="sm:col-span-7 p-5" :class="{ 'sm:col-span-10': !image }">
        <h2
          class="text-xl font-semibold text-black dark:text-zinc-300 pb-1 group-hover:text-teal-500 dark:group-hover:text-sky-400"
        >
          {{ title }}
        </h2>
        ...
        <div
          class="flex group-hover:underline text-teal-500 dark:text-sky-400 items-center pt-2"
        >
          <p>Read More</p>
          <LogoArrow />
        </div>
